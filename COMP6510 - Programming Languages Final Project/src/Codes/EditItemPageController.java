...
public class EditItemPageController implements Initializable {
    ...
    public void initData(String username, ModelTable table){
        this.username = username;
        id = table.getId();
        typeTextField.setText(table.getType());
        itemTextField.setText(table.getItem());
        amountTextField.setText(Integer.toString(table.getAmount()));
        priceTextField.setText(Integer.toString(table.getPrice()));
        date = table.getDate();
    }
    ...
    public void EditButtonClicked(ActionEvent event) {
        ...
        if (isEmpty && isInt){
            ...
            Database.updateItem(username, id, type, item, amount, price, date);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.close();
